...
try
{
  ...
  $bdd = new PDO('mysql:host=localhost;dbname=villever_app0', 'villever_app0', 'Abderrahim@30');
}
...
// On ajoute une entrée dans la table jeux_video
$sql= "UPDATE `villever_app0`.`user_villeverte` SET `nom_user` = '$nom', `email_user` = '$email', `pass_user` = '$password', `tel_user` = '$tel', `nom_residence` = '$nomresidence', `appart_villa` = '$choixbien', `num_immeuble` = '$numimmeuble', `num_appart_villa` = '$numvillaappart', `status` = '$status', `x_user` = '$x', `y_user` = '$y' WHERE `user_villeverte`.`id_user` = '$iduser'";
$req = $bdd->prepare($sql);
$req->execute();
echo trim('ok');
}
...
else if(isset($_POST['afficheservice']))
{
	  ...
	  $sql = 'SELECT * FROM service_parrent where valid_sp=1 and etat_sp= "1" ORDER BY id_sp ASC ';

    foreach ($bdd->query($sql) as $row) {
	?>

  <div style="width:33%;float:left;" class="ui-grid-b">
	<a data-transition="slide" onclick="affiche1('<?php print $row['id_sp']; ?>')" href="#page2"><div style="min-height:115px;" class="biba"><img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_sp']; ?>"><br/><span style="font-weight:bold;"><?php print $row['nom_sp']; ?></span></div></a>
...
$id=$_POST['affichesservice'];
 ...
 if (preg_match("#aucun#", $row['final_ssp'])) { ?> 
 
<div style="min-height:130px;" onclick="addtocart2()" class="biba">
<img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"><br/><span style="font-size:15px;"><?php print $row['nom_ssp']; ?></span><br/></div>

 <?php }


else if (preg_match("#oui#", $row['final_ssp'])) { ?> <a data-transition="slide" href="#page2"> 
 
<div onclick="addtocart('<?php print $row['id_ssp']; ?>', 'http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>','<?php print $row['nom_ssp']; ?>','<?php print $row['prixfinal_ssp']; ?>')"  class="biba">
<img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"><br/><span style="font-size:15px;"><?php print $row['nom_ssp']; ?></span><br/><?php if ($row['final_ssp'] == 'oui') { ?><span style="color:#f86a3b;font-size:15px;font-weight:bold;"><?php print $row['prixfinal_ssp'].'DH'; ?></span><?php } ?></div>
...
else
{
 ...
 <a onclick="affiche111('<?php print $row['cat_ssp2']; ?>')"  data-transition="slide" href="#page222"> 
 
<div style="min-height:130px;" class="biba">
<img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"><br/><span style="font-size:15px;"><?php print $row['nom_ssp']; ?></span><br/></div>
</a> 
...
else if(isset($_POST['affichesservice111']))
{
$id=$_POST['affichesservice111'];
	  $sql = "SELECT * FROM service_sparrent where id_sp2='$id' and valid_ssp=1 ORDER BY id_ssp ASC";
    foreach ($bdd->query($sql) as $row) {
	?>
  <div style="width:33%;float:left;" class="ui-grid-b mim">
 
 <?php
 
 if (preg_match("#aucun#", $row['final_ssp'])) { ?> 

<div onclick="addtocart2()" style="min-height:135px;" class="biba">
<img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"><br/><span style="font-size:15px;"><?php print $row['nom_ssp']; ?></span><br/></div>

<?php }

else if (preg_match("#oui#", $row['final_ssp'])) { ?> <a data-transition="slide" href="#page222">
<div style="min-height:135px;" onclick="addtocart('<?php print $row['id_ssp']; ?>', 'http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>','<?php print $row['nom_ssp']; ?>','<?php print $row['prixfinal_ssp']; ?>')" class="biba">
<img style="width:82px;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"><br/><span style="font-size:15px;"><?php print $row['nom_ssp']; ?></span><br/><?php if ($row['final_ssp'] == 'oui') { ?><span style="color:#f86a3b;font-size:15px;font-weight:bold;"><?php print $row['prixfinal_ssp'].'DH'; ?></span><?php } ?></div>
<div style="display:none;" id="pilo<?php print $row['id_ssp']; ?>"><?php print rtrim($row['descrip_ssp']); ?></div> 
</a> <?php }
...
<div onclick="addtocart('<?php print $row['id_ssp']; ?>', 'http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>','<?php print $row['nom_ssp']; ?>','<?php print $row['prixfinal_ssp']; ?>','')" class="biba">
<div style="display:none;" id="pilo<?php print $row['id_ssp']; ?>"><?php print rtrim($row['descrip_ssp']); ?></div>
<img style="width:100%;" src="http://villeverteapp.com/villeverte/upload/<?php print $row['photo_ssp']; ?>"></div>
...
{
$panier=$_POST['panier'];
$sql="select * from jecommande where clientmonpanier='$panier'";
...
else if(isset($_POST['lepanier']))
{
	...
	$panier=$_POST['panier'];
$id=$_POST['lepanier'];
	  $sql = "select id_cmd, code_panier, id_clientp, id_produitp, qt_produit, etat_cmd, id_ssp, nom_ssp, prixfinal_ssp from service_sparrent, monpanier where id_produitp = id_ssp and code_panier ='$panier' and id_clientp='$client' and etat_cmd='0'";
	  $sql2="SELECT sum(total_product) AS somme FROM monpanier where etat_cmd='0' and code_panier ='$panier' and id_clientp='$client'";
  ...
  <tr class="pr<?php print $row['id_cmd']; ?>">
	...
	<td class="tg-yw4l"><?php print $row['qt_produit']*floatval($row['prixfinal_ssp']); ?></td>
...
<tr class="pr8w">
    ...
    <td  style="background:#f1f1f1;" class="tg-yw4l2"></td>
	 <td  style="background:#f1f1f1;" class="tg-yw4l2">25 DH</td>
    <td  style="background:#f1f1f1;" class="tg-yw4l2"></td>
  </tr>
   ...
   <tr class="pr8w">
    ...
    <td class="tg-yw4l2"></td>
	 <td class="tg-yw4l2"><?php print $row2['somme']+25; ?> DH</td>
    <td class="tg-yw4l2"></td>
  </tr>
...
<table>
   <tr class="pr8w">
    ...
    <td style="width:33%;padding:10px;"  class="tg-yw4l2"><div onclick="jecommande('<?php print $row2['somme'];?>','25')" class="buttonx">Commander</div></td>
	 <td style="width:33%" class="tg-yw4l2"></td>
  </tr>
</table>
<?php
...
$date=date("Y-m-d");
$heure=date("H:i");
$sql = "INSERT INTO `villever_app0`.`jecommande` VALUES (NULL, '$panier', '$client', '$total', '$livraison', '$etat', '$date', '$heure')";
$bdd->exec($sql);
$sql2="UPDATE `villever_app0`.`monpanier` SET `etat_cmd` = '1' WHERE `monpanier`.`code_panier` = '$panier'";
$bdd->exec($sql2);
}
...
$valqt=$_POST['valqt'];
$code_panier=$_POST['codepanier'];
$idclientp=$_POST['idclientp'];
...
$sql = "INSERT INTO monpanier VALUES(null, '$code_panier', '$idclientp', '$idproduct', '$valqt', '$total', '$etat', '$date', '$heure')";
$bdd->exec($sql);

}


else if(isset($_POST['ajouteraupanier2']))
{
$code_panier=$_POST['codepanier'];
$idclientp=$_POST['idclientp'];
$etat="0";
...
$sql = "INSERT INTO demande VALUES(null, '$code_panier', '$idclientp', '$etat', '$demande', '$date', '$heure')";
$bdd->exec($sql);
...
{
$id=$_POST['supppanier'];
$sql = "DELETE FROM `villever_app0`.`monpanier` WHERE `monpanier`.`id_cmd` = $id";
$bdd->exec($sql);
}
  ...
  <div style="margin-top:135px;">
   ...
   <div id="numero2" class="jelly-form" style="width:100%">
    ...
    <input onclick="biza2('nappart')" value="<?php print $row['num_appart_villa']; ?>" type="text" id="nappart" style="border-radius:0px !important;" placeholder="">
			...
			<div style="text-align:center;margin-top:35px;margin-bottom: 50px;padding-bottom:82px;float:left;margin-left:auto;margin-right:auto;width:100%;left:0;right:0;">
